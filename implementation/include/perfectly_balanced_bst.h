...
class PerfectlyBalancedBinaryTree {
 public:
  ...
  NodeType* RecursiveTreeGenerate(std::size_t left, std::size_t right, std::vector<ValueType> & arr) {
    assert(left <= right);
    auto middle = left + (right - left) / 2;
    NodeType* node = new NodeType(arr[middle]);
    if (left != middle) node->left = RecursiveTreeGenerate(left, middle - 1, arr);
    if (right != middle) node->right = RecursiveTreeGenerate(middle + 1, right, arr);
    return node;
  }
