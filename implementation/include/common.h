...
struct Node {
  ...
  Node& operator=(const Node& other) noexcept(
      noexcept(std::is_nothrow_copy_assignable_v<ValueType>)) {
    delete left;
    delete right;
    left = nullptr;
    right = nullptr;
  ...
  Node& operator=(Node&& other) noexcept(
      noexcept(std::is_nothrow_move_assignable_v<ValueType>)) {
...
template <typename T>
const Node<T>* FindByKey(const Node<T>* node,
                         const typename Node<T>::ValueType& key) {
...
template <typename T>
void UniformTraversing(Node<T>* node,
                       std::function<void(typename Node<T>::ValueType&)> func) {
  if (!node) return;
